...
const Posts = ({ posts: propPosts }: PostsProps = {}) => {
  ...
  const handleLikePost = async (postId: string) => {
    ...
    try {
      ...
      const isPagePost =
        originalPost.type === "page_post" ||
        originalPost.page_id ||
        originalPost.page;
        ...
        prevPosts.map((post) =>
          post.id === postId
            ? {
                ...
                like_count:
                  likeCount ??
                  (is_like ? originalCount + 1 : originalCount - 1),
                is_liked: is_like,
              }
            : post
        )
      ...
      <div className="px-0 py-4 h-[calc(100vh-18rem)] overflow-y-scroll">
        <div className="space-y-4">
          {[1, 2, 3].map((i) => (
            <div
              key={i}
              className="bg-white rounded-xl px-4 py-4 shadow-lg animate-pulse mx-0"
            >
  ...
  const fetchReplies = async () => {
    try {
      const response = await api.get(
        `/post/app/comments-replies/?id=${comment.id}`
      );
          ...
          <div className="flex items-center gap-4 text-xs text-gray-500">
            ...
            {comment.comment_reply_count > 0 && (
              <button onClick={handleReplyToggle}>
                {showReplies
                  ? "Hide replies"
                  : `View ${comment.comment_reply_count} replies`}
              </button>
            )}
          </div>
